...
module.exports = async(conn, msg, m, setting, store, welcome, _afk) => {
          try {
                let { ownerNumber, botName, lolkey, xteamkey, gamewaktu, limitCount, packname, author, id_group, xcode } = setting
                   ...
                   return conn.sendMessage(from, {
                     text: teks, contextInfo: {
                      externalAdReply: {
                        ...
                        sourceUrl: `https://instagram.com/wtmsdrjt`
                      }
                     }
                   }, { quoted: msg })
                }
      ...
      if (rumcet !== undefined && rumcet.status === 'CHAT') {
        ...
        if (msg.type == "conversation") {
          ...
          conn.sendMessageFromContent(chat_jid, msg.message, { contextInfo })
                ...
                if (pesanx === 'bot' || pesanx === 'hutao' || pesanx === 'tes' || pesanx === 'p') {
...
*Saldo:* Rp ${toRupiah(blnc.checkBalance(sender, balanceDB))}`)
                ...
                case prefix+'botinfo':
                   ...
                   var capt = `_*Hu Tao Bot Information*_
...
*• Prefix :* Multi Prefix
*• Instagram :* @wtmsdrjt
*• Github :* github.com/wtmsrjt
*• Bot Created On 31 December 2022*
...
*Free :* ${freeGb} GB`
                   textImg(stat)
                   break
                case prefix+'grouphutao': case prefix+'gruphutao':
                case prefix+'grupbot': case prefix+'gchutao':
                   addCountCmd('#grouphutao', sender, _cmd)
                   var link = 'https://chat.whatsapp.com/GB4zdF5UZiW0FcxF6yYpnN'
                   var teks = `Jangan lupa join grup Hu Tao untuk mengetahui informasi lebih lanjut tentang bot\n\nGroup : ${link}\n\nJangan lupa juga untuk Donasi supaya admin semakin semangat dalam mengembangkan bot ini, terimakasih.`
                   textImg(teks)
                   break
                case prefix+'runtime':
...
Haii, kamu bisa Donasi melalui payment OVO/GoPay 085771647181, minimal Donasi Rp1.000 agar hasil donasi kamu bisa lebih membantu untuk membeli server.\n\nTerimakasih karena sudah Donasi, semoga apa yang kamu sumbangkan kepada bot ini, digantikan berkali-kali lipat oleh TUHAN YME. aamiin.`
reply(donasi_capt)
                   break
                case prefix+'owner':
                ...
                case prefix+'say':
                  if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                  if (args.length < 2) return reply(`Kirim perintah ${command} teks`)
                  var req = await (await fetch(`https://api.xcodeteam.xyz/api/converter/text-to-speech?api_key=${xcode}&language=id-ID&text=${q}`)).json()
                  var { code } = req
                  if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                  var { audio_url } = req.data
                  conn.sendMessage(from, { audio: { url: req.data.audio_url }, mimetype: 'audio/mpeg', ptt: true }, { quoted: msg })
                  limitAdd(sender, limit)
                  break
             case prefix+'qc':
                   if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                ...
                case prefix+'openai': case prefix+'ai':
                   ...
                   reply(mess.wait)
                   var req = await (await fetch(`https://api.neoxr.my.id/api/gpt-pro?q=${q}&apikey=Aprilll`))                   
        ...
        reply(`Sukses mengirim pesan Rahasia kepada ${num.split("@")[0]}`)
                ...
                case prefix+'chatwith':
                   ...
                   fs.writeFileSync('./database/chatwith.json', JSON.stringify(chat_with, null, 2))                   
                    var text = `Haii, ada yang ingin chatan sama kamu nih\n\nPesannya :\n${pess}\n\n`
                    text += 'Kamu bisa Terima atau Tolak permintaan chat tersebut dengan cara mengetik salah satu opsi dibawah\n\n'
                    text += `Terima: #acc_chat ${id_chat}\n`
                    text += `Tolak: #dn_acc ${id_chat}`                     
                   conn.sendMessage(nom, text)
                ...
                case prefix+'acc_chat':
                   ...
                   conn.sendMessage(data1.a, `Seseorang yang ingin kamu Chat sudah menerima permintaan Chat kamu, silahkan kirim pesan kepadanya!`)
                   break
                case prefix+'dn_acc':
                   if (!isQuotedMsg) return
                   if (quotedMsg.type !== 'buttonsMessage') return
                   ...
                   if (data2.id !== args[1]) return reply(`Teks yang kamu ketik sudah tidak berlaku lagi!`)
                   if (data2.status === 'CHAT') return reply(`Teks yang kamu ketik sudah tidak berlaku lagi, silahkan ketik ${prefix}batalchat untuk menghentikan sesi chat ini`)
                   var posi2 = chat_with.indexOf(data2)
                   chat_with.splice(posi2, 1)
                   fs.writeFileSync('./database/chatwith.json', JSON.stringify(chat_with, null, 2))
                   reply(`Sukses menolak permintaan Chat tersebut`)
                   conn.sendMessage(data2.a, `Yahh permintaan chat kamu ditolak, NT cuy:(`)
                   break
                case prefix+'batalchat':
                   ...
                   if (cek_data.status === 'WAIT') {
                     ...
                     if (cek_data.a === sender) {
                        ...
                        conn.sendMessage(cek_data.b, `Seseorang telah membatalkan permintaan chatnya denganmu`)
                        ...
                        addCountCmd('#batalchat', sender, _cmd)
                        conn.sendMessage(cek_data.a, `Seseorang telah menolak permintaan chat kamu`)
                        chat_with.splice(posi_x, 1)
                        fs.writeFileSync('./database/chatwith.json', JSON.stringify(chat_with, null, 2))
                     ...
                     if (cek_data.a === sender) {
                       addCountCmd('#batalchat', sender, _cmd)
                       conn.sendMessage(cek_data.b, `Yahh, lawan chat kamu sudah memberhentikan sesi chat ini🥲`)
                       ...
                       addCountCmd('#batalchat', sender, _cmd)
                       conn.sendMessage(cek_data.a, `Yahh, lawan chat kamu sudah memberhentikan sesi chat ini🥲`)
                       chat_with.splice(posi_y, 1)
                       fs.writeFileSync('./database/chatwith.json', JSON.stringify(chat_with, null, 2))
                       reply(`Sukses memberhentikan sesi chat ini`)
                       return
                ...
                case prefix+'tiktok': case prefix+'tt':
                   ...
                   reply(mess.wait)
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/downloader/tiktok?api_key=${xcode}&video_url=${args[1]}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                ...
                case prefix+'tiktokwm':
                   ...
                   if (!args[1].includes('tiktok')) return reply(mess.error.Iv)
                   reply(mess.wait)
                   addCountCmd('#tiktokwm', sender, _cmd)
                   require('../lib/tiktok2').Tiktok(args[1]).then( data => {
                     conn.sendMessage(from, { video: { url: data.watermark }}, { quoted: msg })
                     limitAdd(sender, limit)
                   }).catch(() => reply(mess.error.api))
                   break
                ...
                case prefix+'play':
                   ...
                   reply(mess.wait)
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/downloader/youtube-play-audio?api_key=${xcode}&video_title=${q}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   ...
                   conn.sendMessage(from, { document: req.data.url, fileName: `${title}.mp3`, mimetype: 'audio/mp3'}, { quoted: msg })
                   limitAdd(sender, limit)
                   break
                ...
                case prefix+'grafiti': case prefix+'grafity':
                   if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                   if (args.length < 2) return reply(`Kirim perintah ${command} text1|text2`)
                   if (!q.includes('|')) return reply(`Format salah, contoh pemakaian ${command} Grafity|Hutao`)
                   if (q.length > 21) return reply(`Teksnya kebanyakan`)
                   var tek1 = q.split('|')[0]
                   var tek2 = q.split('|')[1]
                   if (!tek2) return reply(`Format salah, contoh pemakaian ${command} Grafity|Hutao`)
                   if (tek1.length > 10) return reply(`Teks 1 kepanjangan`)
                   if (tek2.length > 10) return reply(`Teks 2 kepanjangan`)
                   reply(mess.wait)
...
*Hasil pencarian dari ${q}*`
                ...
                case prefix+'searchbyimg':
                   ...
                   if (isImage || isQuotedImage) {
                     ...
                     var teks = `*Data Berhasil Di Dapatkan!*\n\n*Url Photo :* ${data.imgUrl}\n\nPencet Link "Url Photo" untuk menuju ke pencarian yang Anda Cari!`                     
...
Oh tidak! Kamu pulang tanpa *${toRupiah(args[1])}* balance (❌)`
        ...
        break
                ...
                case prefix+'tebakkalimat':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/tebak-kalimat?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                ...
                case prefix+'tebakkata':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/tebak-kata?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   var { soal, jawaban } = req.data
                   var teks = `*TEBAK KATA*\n\n`+monospace(`Soal : ${soal}\nPetunjuk : ${jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')}\nWaktu : ${gamewaktu}s`)
                ...
                case prefix+'tebakkimia':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/tebak-kimia?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                ...
                case prefix+'tebaklirik':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/tebak-lirik?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   var { soal, jawaban } = req.data
                ...
                case prefix+'asahotak':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/cak-lontong?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   var { soal, jawaban } = req.data
                   var teks = `*ASAH OTAK*\n\n`+monospace(`Soal : ${soal}\nPetunjuk : ${jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')}\nWaktu : ${gamewaktu}s`)
			...
			case prefix+'family100':
                ...
                var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/family100?api_key=${xcode}`)).json()
                var { code } = req
                if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   var { soal, jawaban } = req.data
                ...
                case prefix+'siapakahaku':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/siapakah-aku?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                   var { soal, jawaban } = req.data
                 ...
                 case prefix+'susunkata':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/susun-kata?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                ...
                case prefix+'tebakbendera':
                   ...
                   var req = await (await fetch(`https://api.xcodeteam.xyz/api/games/tebak-bendera?api_key=${xcode}`)).json()
                   var { code } = req
                   if ([400, 403, 404, 429, 500].includes(code)) return reply(mess.error.api)
                ...
                case prefix+'tictactoe': case prefix+'ttt': case prefix+'ttc':
                   if (!isGroup)return reply(mess.OnlyGrup)
                   if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
                   if (isTicTacToe(from, tictactoe)) return reply(`Masih ada game yang blum selesai`)
                ...
                case prefix+'ttc-solo': case prefix+'tictactoe-solo': case prefix+'ttcsolo':
                   if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
                   if (isTicTacToe(from, ttcsolo)) return reply(`Masih ada game yang blum selesai\nketik ${prefix}endttc untuk menghapus sesi`)
          ...
          .then( res => {
            ...
            gameAdd(sender, glimit)
                ...
                case prefix+'delcasino':
                   ...
                   var teksakinator = `Hai, Saya Akinator!\nPikirkan seorang karakter Fiksi atau Nyata,
Saya akan mencoba untuk menebaknya.\n\n${prefix}akinatorstart -- Untuk Memulai\n${prefix}cancelakinator -- Untuk Menghapus Sesi`
                  reply(teksakinator)
                  break
                case prefix + 'akinatorstart': 
        if (isGroup) return reply(mess.OnlyPM)
        if (akinator.hasOwnProperty(sender.split('@')[0])) return reply("Selesain yang sebelumnya dulu atuh")
        if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
                  ...
                  if (isCmd) {
                    if (args[0].length > 1) {
                      if (isGroup) {
                        ...
                        if (detect !== null) {
                          reply(`Mungkin yang kamu maksud adalah ${prefix + detect} abaikan jika salah!`)
                        }
